...
import { TempleteCard } from "./Cards";
import { Tabs } from "./Tabs";
import { Check } from "lucide-react";
import RenderResume from "./RenderResume";

const TABS_DATA = [{ label: "Templete" }];
...
const ThemeSelector = ({
  ...
  //Selected ThemeTeplete using id
  const initialIndex = resumeTemplates.findIndex((t) => t.id === selectedTheme);
  const [selectedTemplete, setSelectedTemlete] = useState({
    theme: selectedTheme || resumeTemplates[0]?.id || "",
    index: initialIndex >= 0 ? initialIndex : 0,
  });

  const [tabValue, setTabValue] = useState("Templete");

  const handleThemeSelection = () => {
    setSelectedTheme(selectedTemplete.theme);
    onClose();
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 lg:gap-8">
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 p-4 sm:p-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-[60vh] lg:max-h-[70vh] overflow-auto p-2">
            {resumeTemplates.map((template, index) => {
              return (
                <TempleteCard
                  key={`templetes_${index}`}
                  thumbnailImg={template.thumbnailImg}
                  isSelected={selectedTemplete.index === index}
                  onSelect={() =>
                    setSelectedTemlete({
                      theme: template.id,
                      index,
        ...
        >
          <RenderResume
            templateId={selectedTemplete?.theme || ""}
